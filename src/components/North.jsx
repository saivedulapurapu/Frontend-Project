...
function North() {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/Recipes")
      .then((res) => res.json())
      .then((data) => {
        const north = data.filter(
          (r) => r.category?.trim().toLowerCase() === "north indian"
        );
        setRecipes(north);
      })
  ...
  return (
    <>
      ...
      <main className="container py-5" style={{ marginTop: "100px" }}>
        <h2 className="text-center fw-bold mb-4">North Indian Recipes</h2>

        <div className="row g-5 justify-content-center">
          {recipes.length === 0 ? (
            <p style={{ textAlign: "center", color: "#666" }}>
              No North Indian recipes found
            </p>
          ) : (
            recipes.map((recipe) => (
